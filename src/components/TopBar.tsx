...
export default function TopBar() {
  ...
  console.log('TopBar - isAuthenticated:', isAuthenticated, 'user:', user);
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md">
      <div className="container flex items-center gap-4 py-3">
        ...
        <nav className="ml-auto flex items-center gap-1 sm:gap-3">
          ...
          <button 
            onClick={() => {
              console.log('Profile button clicked - isAuthenticated:', isAuthenticated, 'user:', user);
          ...
          >
            ...
            {isAuthenticated && user && (
              <span className="absolute -right-1 -top-1 flex h-3 w-3 items-center justify-center rounded-full bg-green-500" />
            )}
            {hasNewOrder && (
      ...
      />
      {showProfile && (
        <UserProfile isOpen={showProfile} onCloseAction={() => setShowProfile(false)} />
      )}
    </header>
  )
}