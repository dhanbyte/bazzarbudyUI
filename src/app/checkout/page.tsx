...
export default function Checkout(){
  ...
  const { addresses, addAddress, setDefaultAddress } = useAddressBook()
  ...
  useEffect(() => {
    // Only show form if no addresses exist, not if there are addresses available
  ...
  useEffect(() => {
    if (addresses.length > 0) {
      ...
      if (!defaultAddr && addresses[0]) {
        // Auto-set first address as default if none is set
        handleSetDefault(addresses[0].id!);
      }
    }
  }, [addresses]);
  ...
  const handleSuccessfulPayment = async () => {
    ...
    if (!addr || !user) {
      toast({ title: "Error", description: "Could not find address or user to place order." });
      return;
    }
    
    try {
      await placeOrder(user.phoneNumber, items, addr, total, paymentMethod as any)
      clearCart() 
      ...
      toast({ title: "Error", description: "Failed to place order. Please try again." });
    }
  }
  ...
  const handleOnlinePayment = async () => {
    ...
    if (!addr) {
      toast({ 
        title: "पता आवश्यक है", 
        description: "कृपया पहले अपना डिलीवरी पता जोड़ें।" 
      });
      setShowForm(true);
    ...
    try {
      // Create Razorpay order through backend API
        ...
        handler: async function (response: any) {
          try {
            ...
            if (verifyData.success) {
              ...
              toast({ title: "Payment Verification Failed", description: "Your payment could not be verified. Please contact support." });
              setIsProcessing(false);
            }
          } catch (error) {
            console.error('Payment verification error:', error);
            toast({ title: "Verification Error", description: "There was an error verifying your payment. Please contact support." });
            setIsProcessing(false);
          }
        },
        prefill: {
      ...
      const displayMessage = errorMessage.includes("Amount must be at least")
        ...
        : errorMessage;
      toast({ title: "Error", description: displayMessage });
      setIsProcessing(false);
    }
  }
  ...
  const handleSaveAddress = async (addr: Address) => {
    if (user?.phoneNumber) {
      await addAddress(user.phoneNumber, { ...addr, default: addresses.length === 0 || addr.default });
      setShowForm(false);
  ...
  const handleSetDefault = async (addressId: string) => {
    if (user?.phoneNumber) {
        await setDefaultAddress(user.phoneNumber, addressId);
    }
  }
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_360px] md:items-start">
        <div>
          ...
          <div className="card p-4">
            ...
            {!showForm ? (
              ...
              <div className="mt-3">
                <AddressForm 
                  onSubmitAction={handleSaveAddress}
        ...
        <div className="card sticky top-24 p-4">
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm">
            ...
            <p className="text-gray-600 mb-2">Contact us for any queries:</p>
            <div className="flex items-center gap-2">
              <span>📞</span>
              <a href="https://wa.me/919157499884" target="_blank" rel="noopener noreferrer" className="text-brand hover:underline font-medium">
                +91 9157499884
              </a>
            </div>
            <div className="flex items-center gap-2 mt-1">
              <span>✉️</span>
              <a href="mailto:dhananjay.win2004@gmail.com" className="text-brand hover:underline">
